...
@router.get("/user/{public_id}", response_model=schemas.Customer)
def read_user_by_public_id(
    ...
    elif not user.is_customer or current_user.pharmacy_id != user.pharmacy_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
@router.put("/user/{customer_id}/user_voucher", response_model=schemas.Customer)
def apply_voucher(
    customer_id: UUID4,
    current_user: models.User = Depends(deps.get_current_user),
    db: Session = Depends(deps.get_db),
    ...
    """
    Reset the value of the customer's voucher to 0
    """
    if not (current_user.is_admin or current_user.is_owner or current_user.is_employee):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="The user doesn't have enaugh provileges to apply a voucher"
        )
    
    customer = crud.user.get(db, customer_id)
    
    if not customer.is_customer or current_user.pharmacy_id != customer.pharmacy_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Requested user is not a customer or is not a client of this pharmacy"
        )
    
    customer = crud.user.reset_voucher(db, customer)
    return customer
