...
class CRUDOrder(CRUDBase[Order, OrderCreate, OrderUpdate]):
    ...
    def get_multi_placed(self, db: Session, *, skip: int, limit: int, descending: bool = False) -> List[Order]:
        ...
        placed_orders = placed_orders.order_by(Order.order_date.desc()) if descending else placed_orders.order_by(Order.order_date)
    ...
    def get_multi_placed_by_customer(self, db: Session, *, skip: int, limit: int, customer_id: UUID4, descending: bool = False) -> List[Order]:
        ...
        placed_orders = db.query(self.model).filter(Order.user_id == customer_id).filter(Order.status == OrderStatus.placed)
